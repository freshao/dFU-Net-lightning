...
def apply_postprocess_to_folder(input_folder, output_folder):
    maybe_mkdir_p(output_folder)
    input_list = listdir(input_folder)
    # assert len(input_list) > 0
    for case in input_list:
        remove_3d_disconnect(join(input_folder, case), join(output_folder, case), (0, 1, 2))
        remove_3d_disconnect(join(input_folder, case), join(output_folder, case), (1, 2, 0))
        remove_3d_disconnect(join(input_folder, case), join(output_folder, case), (2, 0, 1))


def remove_3d_disconnect(input_file, output_file, axis=(0, 1, 2)):
    '''

    :param input_file:
    :param output_file:
    :param axis: axis[0] 表示对哪个轴取最大连通域
    :return:
    '''
    img_in = sitk.ReadImage(input_file)
    img_npy = sitk.GetArrayFromImage(img_in)
    img_valid = np.sum(img_npy, axis=axis[1:])
    pred = measure.label(img_npy, neighbors=8, connectivity=2)
    start = 0
    sum = 0
    score_list = []
    max_num = 0
    for j in range(1, np.max(pred) + 1):
        if np.sum(pred == j) > max_num:
            max_num = np.sum(pred == j)
            max_pixel = j
        print(np.sum(pred == j), np.sum(pred != 0))
        if np.sum(pred == j) > 0.1 * np.sum(pred != 0):
            pred[pred == j] = max_pixel

    pred[pred != max_pixel]=0
    pred[pred == max_pixel]=1

    # for i in range(img_valid.shape[0]):
    #     if img_valid[i] > 0:
    #         sum += 1
    #     else:
    #         score_list.append((start, sum))
    #         start = i
    #         sum = 0
    # sum = 0
    # start = 0
    ...
    #         sum = i[1]
    #         start = i[0]
    # res = np.zeros_like(img_npy)
    # if axis[0] == 0:
    #     res[start:start + sum, ...] = img_npy[start:start + sum, ...]
    # elif axis[0] == 1:
    #     res[:, start:start + sum, ...] = img_npy[:, start:start + sum, ...]
    # else:
    #     res[:, :, start:start + sum] = img_npy[:, :, start:start + sum]
    # a = res.max()
    # img_out_itk = sitk.GetImageFromArray(res)
    img_out_itk = sitk.GetImageFromArray(pred)
    img_out_itk = copy_geometry(img_out_itk, img_in)
    sitk.WriteImage(img_out_itk, output_file)


if __name__ == "__main__":
    input_folder = '/opt/data/default/medical/data/LITS/result/mce-resimg-5-test12+lits2chaos/nii'
    output_folder = "/opt/data/default/medical/data/LITS/result/mce-resimg-5-test12+lits2chaos/nii_postprocessed_disconnect"
    for_which_classes = [1, ]
    apply_postprocess_to_folder(input_folder, output_folder)

    # list_p = listdir(input_folder)
    # for i in list_p:
    #     apply_postprocessing_to_folder(join(input_folder, i), join(output_folder, i), for_which_classes)
